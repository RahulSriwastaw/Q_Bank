...
export const InputPanel: React.FC<InputPanelProps> = ({ onInputChange, className = '' }) => {
  ...
  const handleFiles = (files: File[]) => {
    ...
    const validFiles = files.filter(file => {
      ...
      if (activeMode === 'pdf') return file.type === 'application/pdf';
      return false;
    });
  ...
  return (
    <div className={`bg-white rounded-2xl border border-slate-200 shadow-sm ${className}`}>
      ...
      <div className="flex border-b border-slate-200">
        ...
        {renderTab('pdf', 'PDF', <FileIcon size={16} />)}
            ...
            >
              ...
              <div className="pointer-events-none">
                ...
                <h3 className="text-lg font-bold text-slate-700 mb-1">
                  {activeMode === 'image' ? 'Drop images here' : 'Drop PDF here'}
                </h3>
                <p className="text-sm text-slate-500 font-medium">
                  or click to browse
                </p>
                <p className="text-xs text-slate-400 mt-4">
                  {activeMode === 'image' ? 'Supports JPG, PNG, WEBP' : 'Supports PDF up to 50MB'}
                </p>
              </div>
            </div>
